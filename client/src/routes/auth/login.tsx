...
function LoginComponent() {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        navigate({
          to:
            stateUser?.userType === 'admin' ? '/admin/dashboard' : '/dashboard',
        })
      } catch (err) {}
    },
    onError: () => {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    try {
      ...
      const stateUser = useAuthStore.getState().user
      navigate({
        to: stateUser?.userType === 'admin' ? '/admin/dashboard' : '/dashboard',
      })
    } catch (err: any) {
      // Error handled by hook
    }
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-lagoon-deep p-12 flex-col justify-between text-white">
        ...
        <div className="relative z-10 max-w-md">
          <h1 className="text-5xl font-bold font-sans leading-tight mb-6">
            Welcome Back to <span className="text-[#4fb8b2]">LYICard</span>
          </h1>
          <p className="text-lg text-white/80 leading-relaxed font-sans">
            Sign in to your account to continue scanning, managing, and sharing
            your digital business cards.
          </p>
        </div>

        <div className="relative z-10 text-sm text-white/60 font-sans">
          © 2024 LYICard. All rights reserved.
        </div>
      </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <Link
                      ...
                      className="text-xs text-[#4fb8b2] hover:text-[#328f97] transition-colors"
                    >
                      Forgot password?
                    </Link>
                  </div>

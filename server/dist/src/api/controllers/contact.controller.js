...
export const updateContact = async (req, res, next) => {
    try {
        ...
        delete updates._id;
        const contact = await Contact.findOneAndUpdate({ _id: id, userId: req.user._id }, { $set: updates }, { new: true });
        if (!contact) {
            res.status(404).json({ error: 'Contact not found' });
            return;
        }
...
export const deleteContact = async (req, res, next) => {
    try {
        const { id } = req.params;
        const contact = await Contact.findOneAndUpdate({ _id: id, userId: req.user._id }, { $set: { isDeleted: true } }, { new: true });
        if (!contact) {
            res.status(404).json({ error: 'Contact not found' });
            return;
        }
...
export const sendDefaultEmail = async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user._id;

...
export class AdminAnalyticsController {
  public getUserAnalytics = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const now = new Date();
  ...
  public getRevenueAnalytics = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

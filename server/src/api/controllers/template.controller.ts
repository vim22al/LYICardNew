  ...
  try {
    ...
    if (isDefault) {
      await Template.updateMany(
        { userId, type: data.type || 'email', isDeleted: false },
        { $set: { isDefault: false } }
      );
    }
...
export const updateTemplate = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const keptAttachments = existingTemplate.attachments.filter(att => 
      existingAttachmentIds.includes(att._id?.toString())
    );
    ...
    if (isDefault) {
      ...
      await Template.updateMany(
        { userId, type, isDeleted: false, _id: { $ne: id } },
        { $set: { isDefault: false } }
      );
    }
